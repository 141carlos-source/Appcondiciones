...
function init(){patch();if(started)return;started=true;setTimeout(patch,300);setTimeout(patch,1200);setTimeout(patch,3000);setInterval(patch,2500)}
window.Soltec135SafeExit={init:init,version:B,salir:function(){let w=frame();if(w)return salirSeguro(w)}};init();
})();