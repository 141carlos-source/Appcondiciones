(function(){'use strict';
const BUILD='V135-AVISOS-SIMPLE5';
...
function patch(){if(!appWindow)appWindow=findWindow();const w=appWindow;if(!w||!w.App)return false;ensureSelector();if(patched)return true;patched=true;const A=w.App,origMode=A.crearAvisoModo&&A.crearAvisoModo.bind(A),origNew=A.nuevoAviso&&A.nuevoAviso.bind(A),origEdit=A.editarAviso&&A.editarAviso.bind(A),origDup=A.duplicarAviso&&A.duplicarAviso.bind(A),origCancel=A.cancelarAviso&&A.cancelarAviso.bind(A),origSave=A.guardarAviso&&A.guardarAviso.bind(A);
 A.crearAvisoModo=function(){const s=el('crearAvisoModo'),v=s?String(s.value||''):'';if(v==='condiciones'){if(origNew)origNew();const f=el('formAviso');if(f)f.dataset.s135TipoNuevo='condiciones';setCondFlag(true);applyMode('full');if(s)s.value='';return}if(v==='nuevo'){if(origNew)origNew();const f=el('formAviso');if(f)f.dataset.s135TipoNuevo='simple';setCondFlag(false);applyMode('simple');if(s)s.value='';return}if(origMode)origMode();if(v==='duplicar')applyMode('full')};
 if(origNew)A.nuevoAviso=function(){const r=origNew();const f=el('formAviso');if(f&&!f.dataset.s135TipoNuevo)f.dataset.s135TipoNuevo='simple';setCondFlag(false);applyMode('simple');return r};
 if(origSave)A.guardarAviso=function(){const f=el('formAviso'),tipo=f&&f.dataset.s135TipoNuevo||'';if(tipo==='condiciones')setCondFlag(true);else if(tipo==='simple')setCondFlag(false);const r=origSave();return r};
 if(origEdit)A.editarAviso=function(id){const r=origEdit(id);const f=el('formAviso');if(f)delete f.dataset.s135TipoNuevo;applyMode('full');return r};
 if(origDup)A.duplicarAviso=function(id){const r=origDup(id);const f=el('formAviso');if(f)delete f.dataset.s135TipoNuevo;applyMode('full');return r};
 if(origCancel)A.cancelarAviso=function(){const r=origCancel();const f=el('formAviso');if(f)delete f.dataset.s135TipoNuevo;applyMode('full');return r};return true}
