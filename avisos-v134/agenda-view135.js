...
function renderMonth(){
  ...
  for(let i=0;i<42;i++){const d=new Date(start);d.setDate(start.getDate()+i);const ds=iso(d),items=all.filter(r=>r.date===ds).sort((a,b)=>String(a.time||'').localeCompare(String(b.time||''))),muted=d.getMonth()!==m;
    html+='<div class="agendaMonthCell '+(muted?'muted ':'')+(ds===today?'today':'')+'" data-agenda-day="'+ds+'"><div class="agendaMonthDate">'+d.getDate()+'</div>'+items.slice(0,3).map(r=>'<div class="agendaMonthJob" data-agenda-open="'+esc(r.id)+'">'+esc(r.time||'')+' '+esc(r.title||'Sin título')+'</div>').join('')+(items.length>3?'<div class="agendaMonthMore">+'+(items.length-3)+' más</div>':'')+'</div>'
  }
