...
function inject(){
  ...
  const side=document.querySelector('.side');if(side&&!$('agendaSummarySection')){
    const summary=document.createElement('details');summary.id='agendaSummarySection';summary.className='agendaSummaryDrop agendaSideSection';summary.open=window.innerWidth>860;summary.innerHTML='<summary>Resumen</summary><div id="agendaSummaryBody" style="padding:0 10px 10px"></div>';
    const filters=document.createElement('details');filters.id='agendaFilterSection';filters.className='agendaFilterDrop agendaSideSection';filters.open=window.innerWidth>860;filters.innerHTML='<summary>Filtros</summary><div id="agendaFilterBody" style="padding:0 10px 10px"></div>';
    const uns=document.createElement('section');uns.id='agendaUnscheduledPanel';uns.className='agendaUnscheduledPanel';uns.innerHTML='<div class="agendaUnscheduledTitle"><strong>Sin planificar</strong><span id="agendaUnscheduledCount" class="agendaUnscheduledCount">0</span></div>';
    side.parentNode.insertBefore(summary,side);side.parentNode.insertBefore(filters,side);side.parentNode.insertBefore(uns,side);
    const sb=$('agendaSummaryBody'),fb=$('agendaFilterBody');
    const alarm=side.querySelector('.alarmBox'),metricNodes=[...side.querySelectorAll('.metric')],loadTitle=[...side.querySelectorAll('h3')].find(n=>/Carga por técnico/i.test(n.textContent||'')),load=$('techLoad');
    const techLabel=$('weekTech')&&$('weekTech').closest('label');
    if(alarm)sb.appendChild(alarm);metricNodes.forEach(n=>sb.appendChild(n));if(loadTitle)sb.appendChild(loadTitle);if(load)sb.appendChild(load);
    if(techLabel)fb.appendChild(techLabel);
    const oldTitle=[...side.querySelectorAll('h3')].find(n=>/Sin planificar/i.test(n.textContent||''));if(oldTitle)oldTitle.remove();
    const unsBox=$('unscheduled');if(unsBox)uns.appendChild(unsBox);
    side.remove();
  }
