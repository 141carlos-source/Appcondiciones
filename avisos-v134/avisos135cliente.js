...
function layout(){loadSalida();let w=frame();if(!w)return;let d=w.document,grid=d.querySelector('#formAviso .grid2');if(!grid||editing(d))return;hideOldFlags(d);let title=Array.from(grid.querySelectorAll('.sectionTitle')).find(function(x){return /FICHA DE CLIENTE|CLIENTE Y SUMINISTRO/i.test(x.textContent||'')});let address=lab(d,'avDireccion'),client=lab(d,'avCliente'),contact=lab(d,'avContacto'),nif=lab(d,'avNif'),phone=lab(d,'avTelefono'),email=lab(d,'avEmail'),ibanLab=lab(d,'s132Iban'),cp=lab(d,'avCp'),local=lab(d,'avLocalidad'),prov=lab(d,'avProvincia'),ref=lab(d,'avRefCatastral'),cupsE=lab(d,'avCupsE'),cupsG=lab(d,'avCupsG');if(!title||!address||!phone||!email||!ibanLab||!cp||!local||!prov)return;let findWrap=d.getElementById('c135FindCpWrap');if(!findWrap){findWrap=d.createElement('div');findWrap.id='c135FindCpWrap';findWrap.className='c135-find-cp'}let localSpacer=d.getElementById('c135LocalSpacer');if(!localSpacer){localSpacer=d.createElement('div');localSpacer.id='c135LocalSpacer';localSpacer.setAttribute('aria-hidden','true')}[address,client,contact,nif,phone,prov,local,cp,email,ibanLab,ref].forEach(function(n){if(n)n.classList.add('wide')});let chain=title;[address,client,contact,nif,phone,prov,local,localSpacer,findWrap,cp,email,ibanLab,ref].forEach(function(n){if(n){after(grid,n,chain);chain=n}});let cli=d.getElementById('s132CliExtra');if(cli)cli.style.display='none';let cart=cartoDetails(d);if(cart){after(grid,cart,chain);chain=cart}let plan=planDetails(d);if(plan){after(grid,plan,chain);chain=plan}let note=cupsG?cupsG.nextElementSibling:null;if(!(note&&note.classList&&note.classList.contains('muted')))note=null;let cupsReq=d.getElementById('s132CupsReq');let cupsWrap=d.getElementById('av135CupsDetails');if(!cupsWrap){cupsWrap=d.createElement('details');cupsWrap.id='av135CupsDetails';cupsWrap.className='c135-tab';cupsWrap.innerHTML='<summary>🔌 CUPS y solicitud de CUPS</summary><div class="c135-tab-body grid2" id="av135CupsBody"></div>'}after(grid,cupsWrap,chain);chain=cupsWrap;let cupsBody=d.getElementById('av135CupsBody');[cupsE,cupsG,note,cupsReq].forEach(function(n){if(n){n.classList.add('wide');cupsBody.appendChild(n)}});let mail=inject(d);if(mail){after(grid,mail,chain);chain=mail}let i=d.getElementById('avCupsE'),j=d.getElementById('avCupsG');if(i){i.placeholder='CUPS electricidad';i.autocomplete='off'}if(j){j.placeholder='CUPS gas';j.autocomplete='off'}done=true}
